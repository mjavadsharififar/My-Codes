...
#this challenga has 2 parts, one for regression and one for classification, using Mushroom dataset. descrieption of challenge is as follows:
...
def answer_two():
    ...
    for degree in range(0,9):
        ...
        X_poly = poly.fit_transform(X_train.reshape(11,1))
        linreg = LinearRegression().fit(X_poly, y_train)
        k1 = linreg.score(X_poly,y_train)
        k2 = linreg.score(poly.fit_transform(X_test.reshape(4,1)),y_test)
        g1 = np.append(g1,[[k1]])
        g2 = np.append(g2,[[k2]])
        g = tuple((g1,g2))
...
def answer_four():
    ...
    linscore = r2_score(y_test,y_pred1)
    
    lasreg = Lasso(alpha = 0.01, max_iter = 10000).fit(X_train_poly,y_train)
    y_pred2 = lasreg.predict(X_test_poly)
    lasscore = r2_score(y_test,y_pred2)
    
    score = ((linscore,lasscore))
